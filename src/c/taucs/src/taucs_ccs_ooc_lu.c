                      ...
                      int** ptrucolcount)
{
    ...
    *ptrpostorder = postorder;
    ...
    for (j=0; j < (*ncols); j++)
    {
...
#ifdef OLD
        {
            ...
            if (nnz != 0 && file != -1 && offset != -1)
            {
                ...
                {
                    taucs_printf("oocsp_colanalyze: lseek failed\n");
                }
                io_size = read(file, tmp_col, nnz * sizeof(int));
                if (io_size != nnz * sizeof(int))
                {
                    taucs_printf("oocsp_colanalyze: Error reading data.\n");
                }
            }
            rowind = tmp_col;
    ...
    if (postnum != *ncols)
    {
        taucs_printf("oocsp_colanalyze: internal error in dfs (2)\n");
    }
    ...
    for (jp=0; jp<(*ncols); jp++)
    {
        ...
        for (ip=0; ip<nnz; ip++)
        {
            ...
            if (p < *ncols)
            {
                stack_push(stack_colptr,stack_colstk,i,p);
            }
        }
...

{
    ...
    *maxsn = 8 - 4; /* 8 is the minimum value we use here */
    ...
    if (*spawidth < 8)
    {
        *spawidth = 8;    /* it might go over the limit */
    }
    ...
    panels         = (int*)taucs_malloc(ncols*sizeof(int));
    (*ptrpanels)     = panels;
    schedstart     = (int*)taucs_malloc(ncols*sizeof(int));
    (*ptrschedstart) = schedstart;
    schedend       = (int*)taucs_malloc(ncols*sizeof(int));
    (*ptrschedend)   = schedend;
    fetchnext      = (int*)taucs_malloc(ncols*sizeof(int));
    (*ptrfetchnext)  = fetchnext;
    ejectnext      = (int*)taucs_malloc(ncols*sizeof(int));
    (*ptrejectnext)  = ejectnext;
    ...
    while (j<ncols)
    {
        ...
        while (stop == 0)
        {
            ...
            if (*spawidth > 0)
            {
                ...
                else if (panelsize + colcount > maxpanelsize)
                {
                    newpanel=1;
                }
            }
...

{
    ...
    for(j = 0; j < panelcols_n; j++)
    {
        ...
        if (   taucs_re(spa[q*nrows + pivotindex]) == 0.0
                && taucs_im(spa[q*nrows + pivotindex]) == 0.0)
        {
            continue;
        }
                   ...
                   )
{
    ...
    */
    ...
    for (i=0; i<ncols; i++)
    {
        if ((int) (schedstart[i]) > nsteps)
        {
            nsteps = (int) schedstart[i];
        }
    }
    ...
    for (step=1; step<=nsteps; step++)
    {
        ...
        if (p==0)
        {
            ...
            for (i=0; i<nrows; i++)
                if (nnzmap[i])
                {
                    taucs_printf("oocsp_numfact: Internal Error (heap not empty; 2)\n");
                }
        }
        else
        {
        ...
        while ((i = num_heap_extractmin( heap, &heapsize, ipivots)) != -1)
        {
            ...
            if (snode_index[k] == snode_last)
            {
                continue;    /* skip rest of supernode */
            }
            ...
            for (tmp = 0; tmp<1; tmp++)
            {
                ...
                for (iip=0; iip<Lclen[k]; iip++)
                {
                    ...
                    if (ii == pivots[k])
                    {
                        srows_n--;    /* I have no idea why this row shows up; always with 0.0 */
                    }
                    if (m2[ii] == -1)
                    {
                        srows[srow_next] = ii;
...
#define OLD_1
                ...
                (schedend[ejectnext[en]]) == step)
        {
            ...
            if (panel_id[p] < 0)
            {
                taucs_printf("oocsp_numfact: internal error (panel stack)\n");
            }
            if (panel_id[p] != j)
            {
            ...
            */
            ...
            if (hash == snode_hash)
            {
                ...
                for (ii=0; ii<lnext; ii++)
                {
                    snode_map[lu_ind[ii]       ] = 1;
                }
                for (ii=0; ii<snode_size; ii++)
                {
                    snode_map[snode_pivrows[ii]] = 1;
                }
                for (ii=0; ii<snode_nnz; ii++)
                    if (snode_map[snode_ind[ii]] != 1)
                    {
                        snode_flag = 0;
                    }
                ...
                for (ii=0; ii<snode_nnz; ii++)
                {
                    snode_map[snode_ind[ii]] = 1;
                }

                /* check that all the column's nonzeros are in the snode   */

                for (ii=0; ii<lnext; ii++) if (snode_map[lu_ind[ii]] != 1)
                    {
                        snode_flag = 0;
                    }
