        ...
        supernodal_factor_matrix* L)
{
    ...
    for (child = first_child[sn]; child != -1; child = next_child[child])
    {
        ...
        else if (recursive_leftlooking_supernodal_factor_llt_ooc(child,
                 FALSE,
                 map,
                 sn_in_core,
                 handle,
        ...
        supernodal_factor_matrix* L)
{
    ...
    for (child = first_child[sn]; child != -1; child = next_child[child])
    {
        child_mem=recursive_compute_supernodes_in_core_old(child,
                  ...
                  sn_in_core,
                  L);
        if ((int)(child_mem+(L->sn_size)[child]*(L->sn_up_size)[child]*sizeof(taucs_datatype))<curr_avail_mem)
        {
            sn_in_core[child] = 1;
        }
    ...
    if (!is_root)
    {
        catch_mem += (L->sn_size)[sn]*(L->sn_up_size)[sn]*sizeof(taucs_datatype);
    }
    return catch_mem;
}
    ...
    for (child = first_child[sn]; child != -1; child = next_child[child])
    {
        if(!sn_in_core[child])
        {
            curr_child_mem = recursive_smart_panelize_ooc_supernodes(child,
                             ...
                             sn_to_panel_map,
                             L);
            if(curr_child_mem > max_child_mem)
            {
                max_child_mem = curr_child_mem;
            }
        }
    }
                    ...
                    for(j=0; j<row_count; j++)
                        for(ir=row_count; ir<M; ir++)
                        {
                            ...
                            L->up_blocks[curr_updated_sn][(bitmap[L->sn_struct[K][first_row+j]]-1)*(L->sn_up_size[curr_updated_sn]-L->sn_size[curr_updated_sn])+(bitmap[L->sn_struct[K][ir+first_row]]-1)] =
                                taucs_sub(L->up_blocks[curr_updated_sn][(bitmap[L->sn_struct[K][first_row+j]]-1)*(L->sn_up_size[curr_updated_sn]-L->sn_size[curr_updated_sn])+(bitmap[L->sn_struct[K][ir+first_row]]-1)] ,
 ...
 supernodal_factor_matrix* L)
{
    ...
    for (child = first_child[sn]; child != -1; child = next_child[child])
    {
        ...
        else if (recursive_leftlooking_supernodal_factor_panel_llt_ooc(child,
                 sn,
                 FALSE,
                 map,
                 sn_in_core,
