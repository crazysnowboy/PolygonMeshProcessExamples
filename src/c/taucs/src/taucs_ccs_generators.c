...
taucs_ccs_matrix* taucs_ccs_generate_mesh2d_negative(int n)
{
    ...
    if (!(m->colptr) || !(m->rowind) || !(m->rowind))
    {
        ...
        taucs_free(m->colptr);
        taucs_free(m->rowind);
        taucs_free(m->values.d/*taucs_values*/);
        return NULL;
    }
    ...
    for (y=0; y<n; y++)
    {
        for (x=0; x<n; x++)
        {
            ...
            {
                (m->rowind)[ip]=i;
                (m->values.d/*taucs_values*/)[ip]=-1.0;
                ip++;
            }
            i=(x  )      + mod(y+1,n)*n ;
            if (i>j)
            {
                (m->rowind)[ip]=i;
                (m->values.d/*taucs_values*/)[ip]=+100.0;
                ip++;
            }
            i=mod(x+n-1,n) + (y  )*n      ;
            if (i>j)
            ...
            (m->values.d/*taucs_values*/)[ip]= 202.0;
            if (x==0 && y==0)
            {
                (m->values.d/*taucs_values*/)[ip] += 1;    /* to make it nonsingular */
            }
            ip++;
        }
    }
        ...
        for (x=0; x<n; x++)
        {
            ...
            if (!strcmp(which,"anisotropic_y"))
            {
                if (y < n-1)
                {
                    i=(x  )+(y+1)*n;
                    (m->rowind)[ip]=i;
                    (m->values.d/*taucs_values*/)[ip]=-jump;
                    ip++;
                }
            }
            else if (y < n-1)
            ...
            if (!strcmp(which,"anisotropic_y"))
            {
                ...
                if (x==0 && y==0)
                {
                    (m->values.d/*taucs_values*/)[ip] += 1.0;    /* to make it nonsingular */
                }
                ip++;
            }
            ...
            else if (!strcmp(which,"dirichlet"))
            {
                i=(x  )+(y  )*n;
                (m->rowind)[ip]=i;
                (m->values.d/*taucs_values*/)[ip]= 4.0;
                ip++;
            }
            else /* neumann */
            {
                    ...
                    int visited[])
{
    ...
    for (jp=0; jp<degree[i]; jp++)
    {
        ...
        if (! visited[j] )
        {
            count += recursive_visit(j,neighbors,degree,visited);
        }
    }
    return count;
}
    ...
    for (z=0; z<Z; z++)
    {
        for (y=0; y<Y; y++)
        {
            for (x=0; x<X; x++)
            {
                ...
                if ( ((double)rand() / (double)RAND_MAX) < drop_probability)
                {
                    jw = -1;
                }
                if ( ((double)rand() / (double)RAND_MAX) < drop_probability)
