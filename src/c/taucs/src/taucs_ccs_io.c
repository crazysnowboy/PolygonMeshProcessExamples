...
taucs_dtl(ccs_read_ijv)(char* ijvfilename,int flags)
{
    ...
    while (!feof(f))
    {
...
#ifdef TAUCS_CORE_DOUBLE
        if (fscanf(f, "%lg %lg %lg", &di, &dj, &dv) != 3)
        {
            break;
        }
#endif
...
#ifdef TAUCS_CORE_SINGLE
        if (fscanf(f, "%lg %lg %g", &di, &dj, &dv) != 3)
        {
            break;
        }
#endif
...
#ifdef TAUCS_CORE_DCOMPLEX
            if (fscanf(f, "%lg %lg %lg+%lgi", &di, &dj, &dv_r,&dv_i) != 4)
            {
                break;
            }
#endif
#ifdef TAUCS_CORE_SCOMPLEX
            if (fscanf(f, "%lg %lg %g+%gi", &di, &dj, &dv_r, &dv_i) != 4)
            {
                break;
            }
#endif
        ...
        if ((flags & TAUCS_HERMITIAN) && is[nnz] < js[nnz])
        {
            continue;
        }
